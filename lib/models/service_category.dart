...
String slaKeyFor(String category, String subType) {
  if (category == 'bass') {
    final medical = ['Medical – Dialysis', 'Medical – Chemotherapy', 'Medical – Major Operations'];
...
List<FormFieldConfig> _documentFields(String subType) {
  switch (subType) {
    case 'Indigency Certificate':
      return const [
        FormFieldConfig(key: 'beneficiaryName', label: 'Name of Beneficiary/Patient', type: FormFieldType.text, required: true),
        FormFieldConfig(key: 'guardianName', label: 'Name of Guardian/Claimant', type: FormFieldType.text, required: true),
        FormFieldConfig(key: 'address', label: 'Address', type: FormFieldType.text, required: true),
        FormFieldConfig(key: 'relationship', label: 'Relationship to Beneficiary/Patient', type: FormFieldType.text, required: true),
        FormFieldConfig(key: 'purpose', label: 'Purpose', type: FormFieldType.textarea, required: true),
      ];
    case 'Barangay ID':
      return const [
        FormFieldConfig(key: 'surname', label: 'Surname', type: FormFieldType.text, required: true),
        FormFieldConfig(key: 'firstName', label: 'First Name', type: FormFieldType.text, required: true),
        ...
        FormFieldConfig(key: 'lengthOfStay', label: 'Length of Stay in Barangay', type: FormFieldType.text, required: true),
        ...
        FormFieldConfig(key: 'emergencyName', label: 'Person to Contact in Case of Emergency: Name', type: FormFieldType.text),
        FormFieldConfig(key: 'emergencyContact', label: 'Emergency Contact No.', type: FormFieldType.phone),
        FormFieldConfig(key: 'emergencyRelation', label: 'Emergency Relation', type: FormFieldType.text),
      ];
    case 'Barangay Clearance':
      return const [
        FormFieldConfig(key: 'fullName', label: 'Full Name / Pangalan', type: FormFieldType.text, required: true),
...
List<FormFieldConfig> _bassFields() {
  return const [
    FormFieldConfig(key: 'patientName', label: 'Patient/Beneficiary Name', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'claimantName', label: 'Claimant Name', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'address', label: 'Address', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'contact', label: 'Contact No.', type: FormFieldType.phone, required: true, hint: '09XXXXXXXXX'),
    FormFieldConfig(key: 'relationship', label: 'Relationship to Patient', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'purpose', label: 'Purpose / Details', type: FormFieldType.textarea, required: true),
    FormFieldConfig(key: 'assistanceAmount', label: 'Assistance Amount (₱)', type: FormFieldType.number),
  ];
}
...
List<FormFieldConfig> _communityFields(String subType) {
  return const [
    ...
    FormFieldConfig(key: 'address', label: 'Address', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'contact', label: 'Contact No.', type: FormFieldType.phone, required: true, hint: '09XXXXXXXXX'),
    FormFieldConfig(key: 'details', label: 'Details / Description', type: FormFieldType.textarea, required: true),
  ];
}
...
List<FormFieldConfig> _vawFields(String subType) {
  return const [
    FormFieldConfig(key: 'reporterName', label: 'Reporter Name (optional — may be anonymous)', type: FormFieldType.text),
    FormFieldConfig(key: 'address', label: 'Address', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'contact', label: 'Contact No.', type: FormFieldType.phone, hint: '09XXXXXXXXX'),
    FormFieldConfig(key: 'incidentDetails', label: 'Incident Details', type: FormFieldType.textarea, required: true),
    FormFieldConfig(key: 'actionRequested', label: 'Action Requested', type: FormFieldType.textarea, required: true),
  ];
}

List<FormFieldConfig> _educationFields() {
  return const [
    FormFieldConfig(key: 'studentName', label: 'Student Name', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'address', label: 'Address', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'contact', label: 'Contact No.', type: FormFieldType.phone, required: true, hint: '09XXXXXXXXX'),
    FormFieldConfig(key: 'school', label: 'School', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'gradeLevel', label: 'Grade Level', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'honorLevel', label: 'Honor Level', type: FormFieldType.dropdown, required: true, options: ['With Highest Honors', 'With High Honors', 'With Honors']),
    FormFieldConfig(key: 'guardianName', label: 'Parent/Guardian Name', type: FormFieldType.text, required: true),
  ];
}
...
List<FormFieldConfig> _genericFields() {
  return const [
    ...
    FormFieldConfig(key: 'address', label: 'Address', type: FormFieldType.text, required: true),
    FormFieldConfig(key: 'contact', label: 'Contact No.', type: FormFieldType.phone, required: true, hint: '09XXXXXXXXX'),
    FormFieldConfig(key: 'details', label: 'Details', type: FormFieldType.textarea, required: true),
  ];
}
...
class BassDocument {
  ...
  BassDocument({required this.name, this.required = true, this.uploaded = false});
}

List<BassDocument> kBassDocuments() => [
  BassDocument(name: "Medical Abstract / Doctor's Certificate / Death Certificate", required: true),
  BassDocument(name: "Voter's Certificate from Comelec (patient)", required: true),
  BassDocument(name: "Voter's Certificate from Comelec (claimant)", required: true),
  ...
  BassDocument(name: "Proof of relationship to patient / demised", required: true),
